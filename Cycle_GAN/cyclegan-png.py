...
image = os.listdir('dataset/cezanne2photo/trainA/')
...
class CycleGanData(Dataset):
    ...
    def trainA(self,trainA_path):
        ...
        trainA = Image.open('dataset/cezanne2photo/trainA/'+trainA_path).convert('RGB')
        IMG_SIZE = 256
        trainA = trainA.resize((420,296)) # (420,297)
    ...
    def trainB(self,trainB_path):
        trainB = Image.open('dataset/cezanne2photo/trainB/'+trainB_path).convert('RGB')
        IMG_SIZE = 256
...
input_A = Tensor(4, 3, 256, 256)
input_B = Tensor(4, 3, 256, 256)
